...
#sqlQuery(odbc_connection, "SELECT * FROM T_Breakage WHERE Warehouse = 'STL' AND Date BETWEEN #06/01/2015# AND #03/31/2016#")
...
#     start_date = reactive({
#       format(input$dates[1])
#     })
#     end_date = reactive({
#       format(input$dates[2])
#     })
#     house = reactive({
#       ifelse(input$house == 'Saint Louis', 'Saint Louis', 'Kansas City')
#     })
    
#     q_brk_stl = reactive({
#       isolate(
#         paste0("SELECT * FROM T_Breakage WHERE Warehouse = 'STL' AND Date BETWEEN '",
#                start_date(), "' AND '", end_date(), "'"))
#     })
#     
#     q_brk_kc = reactive({
#       isolate(
#         paste0("SELECT * FROM T_Breakage WHERE Warehouse = 'KC' AND Date BETWEEN '",
#                start_date(), "' AND '", end_date(), "'"))
#     })
    ...
    output$total_breakage = renderValueBox({
      valueBox(
        ...
        'Warehouse Breakage (Cases)', icon=icon('trash-o')
      )
    })
