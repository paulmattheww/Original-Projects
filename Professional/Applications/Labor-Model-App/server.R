...
shinyServer(
  function(input, output) {
    ...
    monthAvg <- reactive({
      filter(myData, MONTH == input$Month)
    })
    
    avgWorkersMonth <- reactive({
      round(mean(monthAvg$INFERRED.NIGHT.EMPLOYEES), 1)
    })
    
    maxDay <- reactive({
      round(max(monthAvg$INFERRED.NIGHT.EMPLOYEES), 1)
    })
    ...
    output$avgWorkersMonth <- renderText({
      paste('For', month(), 'the average shipping employees needed was')
    })
