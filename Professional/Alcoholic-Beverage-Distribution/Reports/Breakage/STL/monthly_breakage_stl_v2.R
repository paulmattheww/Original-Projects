...
print('Read in STL data; do not change column headers') # CHANGE BELOW FILE PATHS TO STL
print('Next time you run this delete the files at the paths below')
breaks = read.csv("C:/Users/pmwash/Desktop/R_Files/Data Input/Input Files for Reports/Breakage/STL/stl_PWBREAKAGE_STL.csv", header=TRUE) 
sales = read.csv('C:/Users/pmwash/Desktop/R_Files/Data Input/Input Files for Reports/Breakage/STL/stl_sales_history_for_breakage_report.csv', header=TRUE) # ensure it is for correct house
supplier = read.csv('C:/Users/pmwash/Desktop/R_Files/Data Input/Input Files for Reports/Breakage/STL/stl_suppbreaka.csv', header=TRUE)
supplier_history = read.csv('C:/Users/pmwash/Desktop/R_Files/Data Input/Input Files for Reports/Breakage/STL/stl_supplier_history_for_breakage.csv', header=TRUE) # n eed to append 
history = read.csv('C:/Users/pmwash/Desktop/R_files/Data Input/Input Files for Reports/Breakage/STL/stl_breakage_detailed_history.csv', header=TRUE) # need to append from the by class function
...
head(history); names(history)

append_breakage_history = function(history, incident_summary, cost_summary, year=2016, month='May') {
  ...
  names(combined) = c('Type', 'Year', 'Month',
                      'Sales.Incidents', 'Driver.Incidents', 'Warehouse.Incidents',
                      'Columbia.Incidents',
                      'Sales.Cost', 'Driver.Cost', 'Warehouse.Cost', 'Columbia.Cost')
  names(history) = names(combined) 
  
  new_history = rbind(history, combined)
  new_history[, c(4:11)] = sapply(new_history[, c(4:11)], function(x) as.numeric(as.character(x)))

  write.csv(new_history, 'C:/Users/pmwash/Desktop/R_files/Data Input/Input Files for Reports/Breakage/STL/stl_breakage_detailed_history.csv', row.names=FALSE)
...
append_supplier_breakage_history = function(supplier, supplier_history) {
  ...
  write.csv(new_supplier_history, 'C:/Users/pmwash/Desktop/R_Files/Data Input/Input Files for Reports/Breakage/STL/stl_supplier_history_for_breakage.csv', row.names=FALSE)
...
create_supplier_summary = function(supplier_history, month='January', year=2016) {
  
  lastyr = year-1
  s = supplier_history %>% filter(Month == month) %>% 
...
supplier_for_summary = create_supplier_summary(supplier_history, month=this_month, year=this_year)
...
monthly_summary = create_monthly_summary(master_dataset, month=this_month, year=this_year)
...
file_name = 'breakage_report_may_2016.xlsx'
