...
df$Stage4A_RedesignRequestedToRedesignApproval = round(difftime(df$AwaitingRequesterChangeApproval, df$AwaitingArtworkRedesign, units="days"), 1)
...
df$KickoffMonth = as.character(month(df$Incomplete, T, F))
...
df$DayOfRecord = dat = apply(head(the_milestones), 1, function(x) max(x, na.rm=T))
...
Incomplete_month = df_thisyr_lastyr$KickoffMonth = as.character(month(df_thisyr_lastyr$Incomplete, T, F))
...
turn_cols = cbind(df_thisyr$Turnaround_KickoffToShip, 
                  ...
                  df_thisyr$Stage4A_RedesignRequestedToRedesignApproval,
...
names(turnaround) = c('X','Turnaround','Completion','Stage1','Stage2',
                      'Stage3','Stage4','Stage4A','Stage5','Stage6','Stage7')
...
Summary of current jobs in the pipeline. Only jobs that have a Incomplete date yet do not have a Ship date are considered. 
...
The data shown below includes records for this year and last year where *either the Incomplete date's month or the Invoice Date's month are in any of the months so far this year* For example, today is May 1st, then the dataset will contain January through May for both this year and last year. 
...
KC_Incompletes = make_rownames(kickedoff_byhouse_bymo$`KANSAS CITY`, 'KickoffMonth')
STL_Incompletes = make_rownames(kickedoff_byhouse_bymo$`ST LOUIS`, 'KickoffMonth')
MMO_Incompletes = make_rownames(kickedoff_byhouse_bymo$`MID MISSOURI`, 'KickoffMonth')
