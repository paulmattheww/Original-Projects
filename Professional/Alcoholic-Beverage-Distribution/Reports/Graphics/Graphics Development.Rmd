...
date: "Through Wednesday June 21, 2017"
output: 
  pdf_document:
      df_print: "kable" 
      fig_width: 10
...
inpipe_history = df_thisyr_lastyr[order(df_thisyr_lastyr$POSTERJob.ID), PIPECOLZ]

inpipe_history = data.frame(
  inpipe_history %>% 
    group_by(POSTERJob.ID) %>%
    mutate(AssembledandShipped=max(AssembledandShipped, na.rm=T))##na.locf(AssembledandShipped, na.rm=F))#max(AssembledandShipped, na.rm=T)) 
  )
inpipe_history = data.frame(
...
begunlongago_butnotfinished = last2yrs & stillnotdone
...
# head(tail(job_history,200),50)## looks like stragglers are not dropped from dataset
# df[df$POSTERJob.ID==124706, ]## check older projects for why
...
for(i in 1:length(janlast_decthis)){
  for(j in 1:NROW(job_history)){
    ...
    ENDED = ifelse(is.na(ENDS[j])==TRUE, FALSE, as.Date(janlast_decthis[i]) > as.Date(ENDS[j]))
...
The data shown below includes records for this year and last year *throught today*. For example, today is June 20th, then the dataset will contain January through the 171st day of both this year and last year. Both Jobs and Job Items are shown (along with their YOY percent changes). 
