...
## Check the worst ones
# x = df[!is.na(df$IncompleteStatus4), ]
# head(x)
...
df_inlimbo = df[df$JobInLimbo == T, c('Invoice.Date','POSTER.Designer','POSTERJob.ID',
                                      ...
                                      'Incomplete','IncompleteStatus1','IncompleteStatus2','IncompleteStatus3',
                                      'JobInLimbo','IncompleteStatus4')]
...
inpipe_history = inpipe_history %>% 
  group_by(POSTERJob.ID) %>%
  mutate(MaxShip=max(as.numeric(AssembledandShipped), na.rm=T))
