...
df$JobInLimbo = (is.na(df$IncompleteStatus1) == F) & (is.na(df$AssembledandShipped) == T)
df_inlimbo = df[(df$JobInLimbo == T)&(df$YearOfRecord == this_yr), c('Invoice.Date','POSTER.Designer','POSTERJob.ID',
...
redesign_colz = c('ProjectID_LineID', 'POSTERJob.ID', 'Redesign', 'Turnaround_KickoffToShip', 
                  ...
                  'Accessory_or_Print', 'Menu')
redesigns = df_thisyr[, redesign_colz]
...
ggplot(data=redesigns, aes(x=Redesign, y=Turnaround_KickoffToShip, group=Redesign)) +
  geom_violin(aes(fill=Accessory_or_Print)) +
  geom_jitter(alpha=0.02) +
  theme_minimal() +
  theme(axis.text.x=element_text(angle=90, hjust=1)) +
  labs(x='Whether a Job was Redesigned or Not (TRUE = Redesign)', 
       y='Turnaround in Days', title='Comparing Difference in Turnaround b/n Redesign & Non-Redesign Jobs') +
  geom_hline(yintercept=turn_acc, colour='red') +
  geom_hline(yintercept=turn_pri, colour='blue') +
  scale_y_continuous(labels=comma) +
  facet_wrap(~Accessory_or_Print)
```
...
ggplot(data=redesigns, aes(x=Redesign, y=Turnaround_KickoffToShip, group=Redesign)) +
  geom_violin(aes(fill=Menu)) +
  geom_jitter(alpha=0.02) +
  theme_minimal() +
  theme(axis.text.x=element_text(angle=90, hjust=1)) +
  labs(x='Whether a Job was Redesigned or Not (TRUE = Redesign)', 
       y='Turnaround in Days', title='Comparing Difference in Turnaround b/n Menu & Non-Menu Jobs') +
...
Percentages, as it is throughout this document, are shown as %. Total jobs is the sum of all jobs across all individuals for this year.

```{r redesigns_des}
redesign_unique = redesigns[!duplicated(redesigns$POSTERJob.ID), ]

redesigns_bydes = aggregate(Redesign~POSTER.Designer, data=redesign_unique, FUN=sum)
redesigns_bydes = redesigns_bydes[order(redesigns_bydes$Redesign, decreasing=T), ]
redesigns_bydes = redesigns_bydes[redesigns_bydes$POSTER.Designer %in% active_designers, ]
redesigns_bydes$Percent = round(redesigns_bydes$Redesign / N_JOBZ, 3) * 100
rownames(redesigns_bydes) = redesigns_bydes$POSTER.Designer
redesigns_bydes$POSTER.Designer = NULL
names(redesigns_bydes) = c('Number of Jobs Redesigned', 'Percent (%) of Total Jobs YTD')
redesigns_bydes
```
...
Below displays the top 10 salespeople as measured by the number of times they sent jobs back to Graphics for Redesigns.

```{r redesigns_sls}
redesigns_bysls = aggregate(Redesign~Salesperson, data=redesign_unique, FUN=sum)
redesigns_bysls = redesigns_bysls[order(redesigns_bysls$Redesign, decreasing=T), ]
redesigns_bysls = redesigns_bysls[redesigns_bysls$Salesperson %!in% graphics_posterIDs, ]
redesigns_bysls$Percent = round(redesigns_bysls$Redesign / N_JOBZ, 3) * 100
rownames(redesigns_bysls) = redesigns_bysls$Salesperson
redesigns_bysls$Salesperson = NULL
names(redesigns_bysls) = c('Number of Jobs Redesigned', 'Percent Redesigned (%) of Total Jobs YTD')
head(redesigns_bysls, 10)
```

Adjusting for Salesperson's utilization rate of the Graphics team's resources.

```{r sls_plot_reds, fig.height=5, fig.width=9}
ttl_sls = aggregate(POSTERJob.ID~Salesperson, data=redesign_unique, FUN=len_unique)
ttl_sls = ttl_sls[order(ttl_sls$POSTERJob.ID, decreasing=T), ]
ttl_sls = ttl_sls[ttl_sls$Salesperson %!in% graphics_posterIDs, ]
ttl_sls$Percent = round(ttl_sls$POSTERJob.ID / N_JOBZ, 3) * 100

names(ttl_sls) = c('Salesperson','Number of Jobs YTD', 'Percent (%) of Total Jobs YTD')
redesigns_bysls$Salesperson = rownames(redesigns_bysls)
rownames(redesigns_bysls) = NULL

ttl_sls = merge(ttl_sls, redesigns_bysls, how='outer', on='Salesperson')
ttl_sls = ttl_sls[order(ttl_sls$`Number of Jobs YTD`, decreasing=T), ]

ggplot(ttl_sls, aes(x=`Number of Jobs YTD`, y=`Number of Jobs Redesigned`)) +
  geom_point() + 
  geom_text(position='jitter', 
             aes(x=`Number of Jobs YTD`, y=`Number of Jobs Redesigned`, 
                label=Salesperson, size=scale(`Number of Jobs Redesigned`)),
            colour='blue') +
  geom_smooth(se=F, method='lm') +
  theme_minimal() +
  theme(legend.position='none') +
  scale_x_log10() + scale_y_log10() +
  labs(title='YTD Redesigns as a function of Number of Jobs Submitted', x='Log Jobs', y='Log Redesigns') 
```
...
df_inlimbo = df_inlimbo[!duplicated(df_inlimbo$POSTERJob.ID), ]

abandon_sls = aggregate(cbind(IncompleteStatusCount,POSTER.Qty.Ordered) ~ Salesperson, data=df_inlimbo, FUN=sum)
...
### YTD Jobs Placed On-Hold By Salesperson

Below shows jobs that were placed in "Incomplete" status after starting, but were eventually finished.
...
inpipe_history = inpipe_history %>% ## sort for "unique" function to grab the first one
...
STARTS = as.Date(job_history$Incomplete[1:500])  ##[1:500]
ENDS = as.Date(job_history$AssembledandShipped[1:500]) ##[1:500] 

for(i in 1:length(janlast_decthis[1:500])){
  for(j in 1:NROW(job_history)){
    ...
    job_history[j, PIPEDATE] = STARTED - ENDED
  }
}
