    ...
    try({
      if(colname %in% names(new_df)) {
        if((j != 1) & ('Incomplete' %in% colname)){
          ...
          print(paste0(i, j, DAT, 'CANCELLED JOB'))
          new_df[i, 'IncompleteStatus'] = DAT
        } else {
          DAT = extract_dates_fromtext(job_status, i, j)
          print(DAT)
...
df$JobInLimbo = (is.na(df$IncompleteStatus) == F) & (is.na(df$AssembledandShipped) == T)
df_inlimbo = df[df$JobInLimbo == T, c('Invoice.Date','POSTER.Designer','POSTERJob.ID',
                                      'Salesperson','Customer','POSTER.Qty.Ordered',
                                      'Incomplete','IncompleteStatus','JobInLimbo')]
...
#print(paste0('Out of ', N_JOBZ, ' the following (top 10) salespersons had the following percentages of total redesigns:'))

accessories = c('Menu Books','Table Top Wrap','TT A-Frame Holder','TT A-Frame Holder (Holder Only)', 
                'TT Acrylic Stand','TT Acrylic Stand (Stand Only)','TT Flip Stand','TT Flip Stand (Stand Only)',
                'Vivid Board - Dry Erase','Vivid Board - Dry Erase','Light Box')
redesigns$Accessory_or_Print = ifelse(redesigns$POSTER.ITEM.CATEGORY %in% accessories, 'Accessory', 'Print')
...
menus = c('Menu Books','Drink List','Folded Drink List','Tri-Fold Drink List','Folded Menu Cards','Menu Card - Small Format (QTY = Total Pages)')
redesigns$Menu = redesigns$POSTER.ITEM.CATEGORY %in% menus
...
Below displays the top 25 salespeople as measured by the number of times they sent jobs back to Graphics for Redesigns.
...
names(redesigns_bysls) = c('Number of Jobs Redesigned', 'Percent (%) of Total Jobs YTD')
head(redesigns_bysls, 25)
```
...
inpipe_history = df_thisyr_lastyr[, c('POSTERJob.ID','ProjectID_LineID','Incomplete','AssembledandShipped')]

## Process jobs first
job_history = inpipe_history[!duplicated(inpipe_history$POSTERJob.ID), ]
rownames(job_history) = NULL
job_history = job_history %>% arrange(Incomplete, AssembledandShipped) #sort for ix speed

janlast_decthis = seq.Date(from=as.Date(paste0(last_yr,'-01-01')),
                           to=as.Date(Sys.Date()), by='day')
date_history_df = data.frame(PipeDate=janlast_decthis, Started=NA, Ended=NA)
date_history_df = head(date_history_df, 500)

STARTS = as.Date(job_history$Incomplete[1:500])
ENDS = as.Date(job_history$AssembledandShipped[1:500])

for(i in 1:length(date_history_df$PipeDate)){
...
data.frame(rownames=names(PIPE), Pipeline=PIPE)
plot(PIPE)



#### Debugging below using above
item_history = inpipe_history[!duplicated(inpipe_history$ProjectID_LineID), ]
