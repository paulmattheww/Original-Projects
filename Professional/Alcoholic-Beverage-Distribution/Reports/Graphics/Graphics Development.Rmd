...
janlast_decthis = seq.Date(from=as.Date(paste0(last_yr,'-01-01')),
                           to=as.Date(paste0(this_yr,'-12-31')), by='day')
date_history_df = data.frame(PipeDate=janlast_decthis, Started=NA, Ended=NA)
date_history_df = head(date_history_df, 5)

STARTS = as.Date(job_history$Incomplete[1:5])
ENDS = as.Date(job_history$AssembledandShipped[1:5])

for(i in 1:length(janlast_decthis)){
  print('Counting the jobs on date: ')
  print(janlast_decthis[i])
