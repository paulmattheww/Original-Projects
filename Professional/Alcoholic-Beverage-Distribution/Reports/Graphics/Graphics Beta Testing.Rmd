...
colsToKeep = c(unlist(colnames(new_df)), unlist(goodCols))
    ...
    try({
      if(colname %in% names(new_df)) {
        if((j != 1) & ('Incomplete' %in% colname)){
          ...
          new_df[i, paste0('IncompleteStatus',as.character(incompleteStatus))] = DAT
          ...
          new_df[i, paste0('BrandMentionsEntered', as.character(brandMentionsStatus))] = DAT
          ...
          new_df[i, paste0('ArtworkRedesign', as.character(artworkRedesigns))] = DAT
...
new_df = new_df[,names(new_df) %in% colsToKeep]

## Mark Jobs that have reached the maximum character limit of 1032 
## In the POSTER.JOB.STATUS field
new_df$JobStatusFieldAtCapacity = nchar(as.character(new_df$POSTER.JOB.STATUS)) == 1032


###### CHECKING FOR BUGGIES ###### CHECKING FOR BUGGIES
# new_df[!is.na(new_df$BrandMentionsEntered1),]
# new_df[!is.na(new_df$IncompleteStatus1),]
# new_df[!is.na(new_df$ArtworkRedesign5),]
# new_df[is.na(new_df$AssembledandShipped),]

# checkStatus = pipeX$POSTER.JOB.STATUS
# checkOtherStatus = df$POSTER.JOB.STATUS
...
df = new_df
## write.csv(new_df,"c:/users/pmwash/desktop/disposable docs/check for graphics.csv"); rm(new_df)
...
#df$JobAbandoned = abandoned = grepl('zj$', df$POSTER.JOB.STATUS)
...
inPipeline = aggregate(JobFinished ~ POSTERJob.ID, data=df, FUN=max)
inPipeline = unlist(inPipeline[inPipeline$JobFinished == 0, 'POSTERJob.ID'])
df$InPipeline = df$POSTERJob.ID %in% inPipeline

getPipeline = function(df){
  dfPipe = df[(df$InPipeline == T) & (!duplicated(df$POSTERJob.ID)), ]
  today = as.character(Sys.Date())
  colsToCheck = c('POSTER.Designer','POSTERJob.ID','POSTER.JOB.STATUS','Salesperson','Customer','Company','POSTER.Price',
...
<!--          'Incomplete','AssembledandShipped','POSTER.JOB.STATUS', -->
...
<!-- britcheck = britcheck[!duplicated(britcheck$POSTERJob.ID), ] -->
<!-- head(britcheck[,c('POSTERJob.ID','POSTER.JOB.STATUS','Incomplete','AssembledandShipped')], 10) -->
...
inpipe_history = df_thisyr_lastyr[order(df_thisyr_lastyr$POSTERJob.ID), PIPECOLZ] #df_thisyr_lastyr
