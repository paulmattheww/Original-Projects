...
Firstly, it serves as a sanity check of the pipeline numbers above. The pipeline is inferred from "jobs in" and "jobs out", and starts far back in time with the initial number of jobs in the pipeline (and cascades from there). 

Secondly, we may derive important ratios and figures from this data. Throughput is the total number of jobs completed over a given time period. Pipeline is the "inventory" of jobs that have been started but have yet to be finished. Dividing throughput over a given time period over the average "inventory" over that same periood will supply what is termed the "turnover" ratio. This indicates how many times the graphics team has turned its inventory over.
