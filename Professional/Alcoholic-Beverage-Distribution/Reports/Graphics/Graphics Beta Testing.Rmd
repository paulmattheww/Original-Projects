...
df = read_table('N:/Operations Intelligence/Monthly Reports/Graphics/Raw Data/paul.txt.txt', engine='python', doublequote=False, header=0)

col_names = {'ID':'ID',
            'DVMKET':'Warehouse',
            'DVJOBID':'JobId',
            'DVJOBNAME':'JobStartDate',
            'DVREPRNT':'Reprint',
            'DVJBSTAT':'JobStatus',
...
tw_yrs_ago = round(as.numeric(format(Sys.Date(), '%Y')) - 2,0)
...
read_raw_graphics_data = function(python_path){
  ...
  #df = df[df$InvoiceYear %in% c(this_yr, last_yr, tw_yrs_ago, NA), ] #check NA
...
## Mark Jobs that have reached the maximum character limit of 1032 
## In the JobStatus field
# df$JobStatusFieldAtCapacity = nchar(as.character(df$JobStatus)) == 1032


###### CHECKING FOR BUGGIES ###### CHECKING FOR BUGGIES
# df[!is.na(df$BrandMentionsEntered1),]
# df[!is.na(df$IncompleteStatus1),]
# df[!is.na(df$ArtworkRedesign5),]
# df[is.na(df$AssembledandShipped),]

# checkStatus = pipeX$JobStatus
# checkOtherStatus = df$JobStatus
# for(i in 1:length(checkStatus)){
#     print(nchar(as.character(checkStatus[i])))
# }
# for(i in 1:length(checkOtherStatus)){
#     print(1032 >= nchar(as.character(checkOtherStatus[i])))
# }
###### CHECKING FOR BUGGIES ###### CHECKING FOR BUGGIES

## write.csv(df,"c:/users/pmwash/desktop/disposable docs/check for graphics.csv"); rm(df)
...
# names(df)[names(df)=='DVDISTCS'] = 'PosterPrice'#quick bandaid
# ## Get profit as specified by Rachel M.
# df$Profit = round(df$PosterPrice - df$ChargebackTotal, 2)


redesigns_menus_accessories = function(df) {
  ## Mark redesigns, menus & accessories
...
get_this_year_data_only = function(df){
  ## Isolate out last 3 mo from thisyr lastyr data
  incomplete_month = df_thisyr_lastyr$KickoffMonth 
  invoice_month = df_thisyr_lastyr$InvoiceMonth
  Incomplete_year = df_thisyr_lastyr$KickoffYear 
  invoice_year = df_thisyr_lastyr$InvoiceYear    
  startmo = incomplete_month %in% last_three_months
  endinmo = invoice_month %in% last_three_months
  trailing_3 = df_thisyr_lastyr[startmo==T | endinmo==T, ]
  df_thisyr = df[(df$KickoffYear == this_yr) | (df$InvoiceYear == this_yr) & (df$Designer %in% active_designers), ]
  return(df_thisyr)
}

df_thisyr = get_this_year_data_only(df)


# ## For below, way below
# byrequester = aggregate(cbind(JobId, JobItemId) ~ Salesperson, data=df_thisyr, FUN=len_unique)
...
# bygraphics = bygraphics[order(bygraphics$JobId, decreasing=T),]
# byrequester = byrequester[order(byrequester$JobId, decreasing=T),]
# 
...
redesign_colz = c('JobItemId', 'JobId', 'Redesign', 'JobTurnaround', 
                  'Reprint', 'Salesperson', 'ItemCategory', 'Designer',
                  'Accessory', 'Menu', 'YearOfRecord')
redesigns = df_ytd_thislast[, redesign_colz]
count_jobs = len_unique(redesigns$JobId)

redesigns$JobID_Redesign = paste0(redesigns$JobId, ' ', redesigns$Redesign)
redesigns$LineID_Redesign = paste0(redesigns$JobItemId, ' ', redesigns$Redesign)


# ## Turnaround later on
# turn_cols = cbind(df_thisyr$JobTurnaround)
# turnaround = aggregate(turn_cols ~ df_thisyr$Designer, FUN=function(x) round(mean(as.numeric(x), na.rm=T), 1))
...
<!-- reprints = df[no_dupes, c('JobId','Designer','POSTER.REPRINT','Year')] -->
<!-- reprints = reprints[reprints$Year == this_yr, ] -->
<!-- reprints = aggregate(POSTER.REPRINT ~ Designer, data=reprints, FUN=sum) -->
...
```{r viz_redesigns_accessoryvprint, fig.height=10, fig.width=9}
turn_acc = mean(redesigns[redesigns$Accessory_or_Print=='Accessory', 'JobTurnaround'], na.rm=T)
turn_pri = mean(redesigns[redesigns$Accessory_or_Print=='Print', 'JobTurnaround'], na.rm=T)

ggplot(data=redesigns, aes(x=Redesign, y=JobTurnaround, group=Redesign)) +
...
```{r men_non_violin, fig.height=10, fig.width=9}
turn_menu = mean(redesigns[redesigns$Menu == T, 'JobTurnaround'], na.rm=T)
turn_nonm = mean(redesigns[redesigns$Menu == F, 'JobTurnaround'], na.rm=T)
...
```{r redesigns_des}
redesign_unique = redesigns[!duplicated(redesigns$JobId), ]

redesigns_bydes = aggregate(Redesign~Designer+YearOfRecord, data=redesign_unique, FUN=sum)
redesigns_bydes = redesigns_bydes[redesigns_bydes$Designer %in% active_designers, ]
...
rownames(redesigns_bydes) = redesigns_bydes$Designer
redesigns_bydes$Designer = NULL
...
```{r multiple_onholds}
onholdcolz = c('YearOfRecord','JobId','Salesperson','Customer','ItemCategory',
...
onhold = onhold[!duplicated(onhold$JobId), ]
YR = onhold$YearOfRecord
SLS = onhold$Salesperson
JID = onhold$JobId
rownames(onhold) = onhold$JobId = onhold$YearOfRecord = onhold$Salesperson = NULL
...
<!-- df_inlimbo_x = df_inlimbo[!duplicated(df_inlimbo$JobId), ] -->
...
```{r limbo_sls}
limbo_colz = c('Salesperson','Warehouse','Year','Customer','IncompleteStatusCount','JobId',
...
limbo_sls_thisyr = limbo_sls_thisyr[!duplicated(limbo_sls_thisyr$JobId), ]
...
val_cols = c('PosterPrice','POSTER.MB.mentions')
id_cols = c('Designer','Month')

bydesigner_melt = melt(df_thisyr, id_cols, val_cols)

bydesigner = aggregate(value ~ variable + Designer + Month, data=bydesigner_melt, FUN=sum)
bydesigner = spread(bydesigner, 'Month', 'value')

l_dfs = split(bydesigner, bydesigner$variable)
price_df = l_dfs$PosterPrice

library(scales)
byd_df = bydesigner
namz = names(byd_df)
namz = c(unlist(namz[namz %!in% c('variable','Designer')]))
mentions_df_melt = melt(byd_df, c('variable','Designer'), namz)
names(mentions_df_melt) = c('var','Designer','Month','value')

ggplot(mentions_df_melt, aes(x=Month, y=value, group=Designer)) +
...
jobspermo = aggregate(JobId ~ Designer + Month, data=df_thisyr, FUN=len_unique)
jobspermo = spread(jobspermo, 'Month', 'JobId')
...
<!--          'Designer','in_pipeline','POSTER.MB.mentions', -->
<!--          'Incomplete','AssembledandShipped','JobStatus', -->
<!--          'IncompleteStatusCount') -->

<!-- inpipe = df_thisyr[(df_thisyr$in_pipeline==T), colz]  -->
...
<!-- price_summary = aggregate(cbind(PosterPrice, POSTER.MB.mentions) ~ Designer, -->
...
inpipe_history = df_thisyr_lastyr[order(df_thisyr_lastyr$JobId), PIPECOLZ] #df_thisyr_lastyr

inpipe_history = data.frame(
  inpipe_history %>% 
    group_by(JobId) %>%
    mutate(AssembledandShipped=na.locf(AssembledandShipped, na.rm=F))
      #max(AssembledandShipped, na.rm=T))##na.locf(AssembledandShipped, na.rm=F))#max(AssembledandShipped, na.rm=T)) 
  )
inpipe_history = data.frame(
...
#job_history = job_history[!is.na(job_history$Incomplete), ]

#############CHANGE IF NEED BE, ALL 3 YEARS IS CONSERVATIVE this_yr 
last2yrs = (lubridate::year(job_history$Incomplete) %in% c(last_yr,tw_yrs_ago))###this_yr
stillnotdone = (is.na(job_history$AssembledandShipped))
begunlongago_butnotfinished = last2yrs & stillnotdone#stillnotdone#
job_history = job_history[!begunlongago_butnotfinished, ]
rownames(job_history) = NULL

## Loop through our data, then the vbector of dates since last year
## To quantify jobs started but not finished
janlast_decthis = seq.Date(from=as.Date(paste0(last_yr,'-01-01')), #this_yr
...
kickedoff_byhouse = kickedoff_byhouse[kickedoff_byhouse$KickoffYear != tw_yrs_ago, ]
...
kickedoff_byhouse_bymo = kickedoff_byhouse_bymo[kickedoff_byhouse_bymo$KickoffYear != tw_yrs_ago & 
...
profit_byhouse = profit_byhouse[profit_byhouse$KickoffYear != tw_yrs_ago, ]
...
jobs_byaccount = jobs_byaccount[jobs_byaccount$KickoffYear != tw_yrs_ago, ]
...
profit_byaccount = profit_byaccount[profit_byaccount$KickoffYear != tw_yrs_ago, ]
...
jobs_bysupplier = jobs_bysupplier[jobs_bysupplier$KickoffYear != tw_yrs_ago, ]
...
profit_bysupplier = profit_bysupplier[profit_bysupplier$KickoffYear != tw_yrs_ago, ]
...
```{r brands_customers}
brands = df_thisyr[,c('Brand','Supplier','ChargebackTotal','PosterPrice',
                            'ItemCategory','JobId','JobItemId')]
brands = brands[brands$Brand != '', ]
b1 = aggregate(cbind(ChargebackTotal, PosterPrice) ~ Supplier + Brand, data=brands, FUN=function(x) round(sum(x, na.rm=T), 0))
b1$Profit = round(b1$PosterPrice - b1$ChargebackTotal, 0)
b2 = aggregate(cbind(JobId, JobItemId) ~ Supplier + Brand, data=brands, FUN=len_unique)
