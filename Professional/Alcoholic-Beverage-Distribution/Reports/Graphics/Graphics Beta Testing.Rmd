...
df = read_table('N:/Operations Intelligence/Monthly Reports/Graphics/Raw Data/PaulThree.txt', engine='python', doublequote=False, header=0)
...
df_ytd_thislast = df_thisyr_lastyr[df_thisyr_lastyr$DOTY %in% days_so_far, ]
...
getRedesigns = function(df_ytd_thislast){
...
redesigns = getRedesigns(df_ytd_thislast)
...
## Turnaround later on
getTurnaround = function(df_ytd_thislast, col_group){
  df_ytd_thislast = df_ytd_thislast[df_ytd_thislast$Designer %in% active_designers, ]
  turnaround = aggregate(df_ytd_thislast$JobTurnaround ~ df_ytd_thislast[, col_group], FUN=function(x) round(mean(as.numeric(x), na.rm=T), 1))
...
turn_menu = mean(df_thisyr_lastyr[df_thisyr_lastyr$Menu == T, 'JobTurnaround'], na.rm=T)
turn_nonm = mean(df_thisyr_lastyr[df_thisyr_lastyr$Menu == F, 'JobTurnaround'], na.rm=T)

#AccessoryPrintBanner

ggplot(data=df_thisyr_lastyr[df_thisyr_lastyr$Designer %in% active_designers, ], aes(x=Menu, y=JobTurnaround, group=Designer)) +
  geom_bar(stat='identity', position='dodge', aes(fill=Designer)) +
  geom_jitter(alpha=0.02) +
  theme_minimal() +
  theme(axis.text.x=element_text(angle=90, hjust=1)) +
  labs(x='TRUE if Job Redesigned', 
