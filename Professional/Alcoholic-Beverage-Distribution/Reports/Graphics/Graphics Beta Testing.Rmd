...
import time
df = read_table('N:/Operations Intelligence/Monthly Reports/Graphics/Raw Data/POSTER Detail_dump.txt', engine='python', doublequote=False, header=0, sep=',')
...
read_raw_graphics_data <- function(python_path){
  df <- read.csv(python_path, header=TRUE)
...
getTurnaround <- function(df, col_group){
  df <- df[df$Designer %in% active_designers, ]
  turnaround <- aggregate(df$JobTurnaround ~ df[, col_group] + df$YearOfRecord, FUN=function(x) round(mean(as.numeric(x), na.rm=T), 1))
  names(turnaround) <- c(col_group, 'Year', 'Turnaround')
  turnaround <- turnaround[order(turnaround$Turnaround, decreasing=T), ]
  turnaround <- tidyr::spread(turnaround, 'Year', 'Turnaround')
  rownames(turnaround) <- turnaround[, col_group]
  turnaround[, col_group] <- NULL
  turnaround$PctChange <- pct_chg(turnaround[, as.character(this_yr)], turnaround[, as.character(last_yr)])
  turnaround <- turnaround[order(turnaround[, as.character(this_yr)], decreasing=T), ]
  turnaround
}

turnaround <- getTurnaround(df_ytd_thislast, 'Designer')
turnaround_sls <- head(getTurnaround(df_ytd_thislast, 'Salesperson'), 25)
turnaround_cus <- head(getTurnaround(df_ytd_thislast, 'Customer'), 25)
turnaround_hse <- head(getTurnaround(df_ytd_thislast, 'Warehouse'), 25)
turnaround_acc <- head(getTurnaround(df_ytd_thislast, 'AccessoryPrintBanner'), 25)
```
...
Below shows jobs that were entered by the Graphics team on behalf of the sales team. Data shown below is for this year and last year, year-to-date.
...
Data considered is from this year and last year.
...
plot_turnaround <- function(turnaround, group_col){
  turnaround$group <- factor(row.names(turnaround), levels=row.names(turnaround))
  if (group_col != 'Designer') {
    turnaround <- turnaround[!is.na(turnaround[, 'PctChange']), ]  
  }
  row.names(turnaround) <- turnaround$PctChange <- NULL
  turnaround <- reshape2::melt(turnaround, 'group')
  
  ggplot(turnaround, aes(x=group, y=value, group=variable)) +
    geom_bar(stat='identity', position='dodge', aes(fill=variable), colour='black') +
    geom_bar(data=turnaround, stat='identity', position='dodge', 
             aes(x=group, y=value, fill=variable), colour='black') +
    theme_minimal() + #facet_wrap(~group_col, nrow=1) +
    theme(axis.text.x=element_text(angle=90, hjust=1)) +
    labs(x='', y='Days', title='Average Turnaround (Days)') +
    coord_flip()
}


plot_turnaround(turnaround, 'Designer')
```

## Turnaround by Salesperson, Customer & House

All other caveats above hold for the plots below, with the added fact that anybody who did not have Jobs active in either last year or this year are filtered out of the data on the visualizations

Longest (this year) turnaround by Salesperson.

```{r turnaround_plot_sls, fig.height=5, fig.width=9}
plot_turnaround(turnaround_sls, 'Salesperson')
```

Longest (this year) turnaround by Customer.
```{r turnaround_plot_cus, fig.height=5, fig.width=9}
plot_turnaround(turnaround_cus, 'Customer')
```

Turnaround by house.
```{r turnaround_plot_hse, fig.height=5, fig.width=9}
plot_turnaround(turnaround_hse, 'Warehouse')
```



## Turnaround by Warehouse, Redesigns, Reprints & Menus
...
Data below contains only records up until today for both this year and last year.

### Redesigns for Accessories v. Print Jobs

Average turnaround: Menus vs. Non-Menus

```{r redesign_plt, fig.height=5, fig.width=9}
designer_summary <- function(df, col, active_designers, FUN, agg_col='JobTurnaround'){
  ...
  turnaround_summary <- turnaround_summary[order(turnaround_summary[, col], turnaround_summary[, 'Designer'], decreasing=TRUE), ]
  turnaround_summary[, col] <- factor(turnaround_summary[, col], levels=unique(turnaround_summary[, col]))
...
plot_redesigns <- function(TurnaroundData, col, title) {
  plt <- ggplot(data=TurnaroundData, 
    ...
    geom_bar(stat='identity', position='dodge', aes(fill=TurnaroundData[,col]), colour='black') +
...
Data considered is YTD for both this year and last year (e.g. 1/1/last_year - 8/5/last_year and 1/1/this_year - 8/5/this_year).
...
```{r sls_plot_reds, fig.height=10, fig.width=9}
...
Below considers jobs that the designer modified during the month in question. If they were assigned a job but never touched it during this period, then it will not show up below.
...
```{r bydes_inpipe, fig.height=6, fig.width=9}
...
throughput_out$`Percent Change (%)` <- pct_chg(throughput_out[, as.character(this_yr)], throughput_out[, as.character(last_yr)])
...
newjobs_out$`Percent Change (%)` <- pct_chg(newjobs_out[, as.character(this_yr)], newjobs_out[, as.character(last_yr)])
...
Below is the first a new addition to the report.

Firstly, it serves as a sanity check of the pipeline numbers above. The pipeline shown below is inferred from "jobs in" and "jobs out", and starts far back in time with the initial number of jobs in the pipeline (and cascades from there). The pipeline shows above uses a heuristic, and actually checks each date in the past whether or not a given job is active (started, not finished), then counts up all active jobs for each date. Whereas the pipeline above is deductive and measured, below is inductive and inferred. They should match up.

Secondly, we may derive important ratios and figures from this data. 

  - Throughput is the total number of jobs completed over a given time period. 
  - Pipeline is the "inventory" of jobs that have been started but have yet to be finished. 
  - Turnover is derived by dividing throughput over a given time period over the average "inventory" over that same periood. Turnover indicates how many times the graphics team can be expected to turn over its inventory over over a given period.

```{r jobsin_jobsout, fig.height=8, fig.width=9}
...
ggplot(plt_data1, aes(x=Month, y=value, group=variable)) + 
  geom_bar(stat='identity', position='dodge', colour='black', 
           aes(fill=variable, group=Year), size=1, alpha=0.8) +
...
```{r jobsin_jobsout_fluctuations, fig.height=5, fig.width=9}
...
ggplot(plt_data2, aes(x=Month, y=value, group=Month)) + 
  geom_line(alpha=0.15, size=2.5) +
  geom_point(aes(colour=variable), size=6, shape=95) + 
  facet_grid(~ Year) +
  theme_minimal() +
  theme(axis.text.x=element_text(angle=90, hjust=1)) +
...
Data below is from this year only.
