...
get_milestones = function(df){
  ...
  df$DOTY = lubridate::yday(df$DayOfRecord)
  return(df)
}
...
## Turnaround later on
getTurnaround = function(df_thisyr){
  turnaround = aggregate(df_thisyr$JobTurnaround ~ df_thisyr$Designer, FUN=function(x) round(mean(as.numeric(x), na.rm=T), 1))
...
turnaround = getTurnaround(df_thisyr)
