...
read_raw_graphics_data = function(python_path){
  ...
  df = df[df$InvoiceYear %in% c(this_yr, last_yr, tw_yrs_ago), ]
  return(df)
}
...
#df$JobAbandoned = abandoned = grepl('zj$', df$JobStatus)
#dfLimboJobs = df[df$JobInLimbo == F, ] 
#dfAbandonedJobs = df[df$JobAbandoned == T, ]
...
# df$Stage4A_RedesignRequestedToResdesignComplete = round(difftime(df$AwaitingArtworkRedesign1, df$AwaitingArtwork, units="days"), 1)
...
inPipeline = aggregate(JobFinished ~ JobId, data=df, FUN=max)
inPipeline = unlist(inPipeline[inPipeline$JobFinished == 0, 'JobId'])
df$InPipeline = df$JobId %in% inPipeline

getPipeline = function(df){
  dfPipe = df[(df$InPipeline == T) & (!duplicated(df$JobId)), ]
  today = as.character(Sys.Date())
  colsToCheck = c('POSTER.Designer','JobId','JobStatus','Salesperson','Customer','Company','POSTER.Price',
...
pipeX = getPipeline(df)

## Mark redesigns
df$Redesign = df$RedesignCounts >= 1
...
getCommonColumns = function(df, string) names(df)[grepl(string, names(df))]
brandMentionsColumns = getCommonColumns(df,'BrandMentions')
df$AllBrandMentionsInput = apply(df[, brandMentionsColumns], 1, function(x) as.POSIXct(max(x, na.rm=T)))
...
# LIMBOCOLZ = c('Invoice.Date','POSTER.Designer','JobId',
...
## END normal data (ie transactional info)
...
## For below, way below
byrequester = aggregate(cbind(JobId, ProjectID_LineID) ~ Salesperson, data=df_thisyr, FUN=len_unique)
...
redesign_colz = c('ProjectID_LineID', 'JobId', 'Redesign', 'Turnaround_KickoffToShip', 
...
N_JOBZ = len_unique(redesigns$JobId)
...
<!-- ```{r reprints} -->
<!-- reprint_cols = c('JobId','POSTER.Designer') -->
<!-- no_dupes = !duplicated(df[,reprint_cols]) -->

<!-- reprints = df[no_dupes, c('JobId','POSTER.Designer','POSTER.REPRINT','Year')] -->
<!-- reprints = reprints[reprints$Year == this_yr, ] -->
<!-- reprints = aggregate(POSTER.REPRINT ~ POSTER.Designer, data=reprints, FUN=sum) -->

<!-- tot_jobs = aggregate(JobId ~ POSTER.Designer, data=df_thisyr, FUN=len_unique) -->
...
```{r multiple_onholds}
onholdcolz = c('YearOfRecord','JobId','Salesperson','Customer','POSTER.ITEM.CATEGORY',
...
```{r limbo_sls}
limbo_colz = c('Salesperson','Company','Year','Customer','IncompleteStatusCount','JobId',
...
jobspermo = aggregate(JobId ~ POSTER.Designer + Month, data=df_thisyr, FUN=len_unique)
...
<!-- britcheck = inpipe[inpipe$POSTER.Designer == 'Riehlman, Brittany', ] -->
<!-- britcheck = britcheck[!duplicated(britcheck$JobId), ] -->
<!-- head(britcheck[,c('JobId','JobStatus','Incomplete','AssembledandShipped')], 10) -->

<!-- pipeline_summary = aggregate(cbind(JobId, ProjectID_LineID) ~ POSTER.Designer, -->
...
PIPECOLZ = c('JobId','ProjectID_LineID','Incomplete','AssembledandShipped')
...
jobtypes = df_thisyr[ , c('POSTER.ITEM.CATEGORY','POSTER.Price',
                          'JobId','ProjectID_LineID','Turnaround_KickoffToShip')]
...
jt4 = aggregate(cbind(JobId, ProjectID_LineID) ~ AccessoryOrPrint, 
...
jt1 = aggregate(cbind(Turnaround_KickoffToShip, POSTER.Price) ~ POSTER.ITEM.CATEGORY, 
                data=jobtypes, FUN=function(x) round(mean(x,na.rm=T), 1))
jt2 = aggregate(cbind(JobId, ProjectID_LineID) ~ POSTER.ITEM.CATEGORY, 
                data=jobtypes, FUN=len_unique)
...
menutypes = df_thisyr[ , c('POSTER.ITEM.CATEGORY','POSTER.Price',
                          'JobId','ProjectID_LineID','Turnaround_KickoffToShip','Menu')]
...
jt6 = aggregate(cbind(JobId, ProjectID_LineID) ~ Menu, 
...
kickedoff_byhouse = aggregate(JobId ~ KickoffYear + Company, data=df_ytd_thislast, FUN=len_unique)
...
kickedoff_byhouse_bymo = aggregate(cbind(JobId,ProjectID_LineID) ~ KickoffYear + KickoffMonth + Company, 
...
brands = df_thisyr[,c('Brand','Supplier','POSTER.CB.Total','POSTER.Price',
                            'POSTER.ITEM.CATEGORY','JobId','ProjectID_LineID')]
...
b2 = aggregate(cbind(JobId, ProjectID_LineID) ~ Supplier + Brand, data=brands, FUN=len_unique)
