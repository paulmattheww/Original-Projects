...
agg_funcs = {'OffDayDelivery' : {'CountOffDayDeliveries':sum, 'CountDeliveries':len}, 
             'Dollars' : {'SumDollars':sum, 'AverageDollars':np.mean}, 
             'Cases' : {'SumCases':sum, 'AverageCases':np.mean}}

DataFrame(deliveries.groupby(['CustomerId','Customer','Week']).agg(agg_funcs)).reset_index(drop=True)

#deliveries[deliveries['OffDayDelivery'] == True]
