...
names(salesTakes) <- c('DATE', 'WAREHOUSE', 'SALES.TAKES')
library(lubridate)
...
salesTakes <- filter(salesTakes, WAREHOUSE == 'KANSAS CITY')
...
kcWarehouseDayHours <- filter(hours, rule1 == "KC Warehouse Wages" |
                                rule1 == "KC Warehouse Casual" |
                                rule1 == "KC Warehouse Non-Union 10" |
                                rule1 == "KC Warehouse Non-Union 8")

personHours <- aggregate(HOURS ~ DATE + PERSONNUM, data=kcWarehouseDayHours, FUN=sum)

day.hours.kc <- aggregate(HOURS ~ DATE, data = personHours, FUN=sum)
day.hours.kc$DATE <- as.character(strptime(day.hours.kc$DATE, "%m/%d/%Y"))
names(day.hours.kc) <- c("DATE", "TOTAL.DAY.HOURS")

day.count.kc <- aggregate(HOURS ~ DATE, data = personHours, FUN=length)
day.count.kc$DATE <- as.character(strptime(day.count.kc$DATE, "%m/%d/%Y"))
names(day.count.kc) <- c("DATE", "NUMBER.DAY.EMPLOYEES")

check <- merge(day.hours.kc, day.count.kc, by="DATE")
head(check, 50)


avg.day.hours <- aggregate(HOURS ~ DATE, data = personHours, FUN=mean)
avg.day.hours$DATE <- as.character(strptime(avg.day.hours$DATE, "%m/%d/%Y"))
names(avg.day.hours) <- c("DATE", "AVG.DAY.HOURS")
head(avg.day.hours)
...
omega.kc <- merge(omega.kc, salesTakes, by="DATE")
omega.kc <- merge(omega.kc, check, by="DATE")
omega.kc <- merge(omega.kc, avg.day.hours, by="DATE")
...
weekdayDummies <- dummy('WEEKDAY',omega.kc)
omega.kc <- cbind(omega.kc, weekdayDummies)
seasonDummies <- dummy('SEASON', omega.kc)
omega.kc <- cbind(omega.kc, seasonDummies)
monthDummies <- dummy('MONTH', omega.kc)
omega.kc <- cbind(omega.kc, monthDummies)
...
omega.kc$INFERRED.DAY.EMPLOYEES <- round(omega.kc$TOTAL.DAY.HOURS / dayHoursAssumed, 1)
omega.kc$INFERRED.NIGHT.EMPLOYEES <- round(omega.kc$TOTAL.HOURS.WORKED / nightHoursAssumed, 1)

check <- cbind(omega.kc$INFERRED.NIGHT.EMPLOYEES, omega.kc$NUMBER.OF.EMPLOYEES)
check
...
kcMonthlyCases <- aggregate(TOTAL.CASES.PRODUCED.KC ~ MONTH, data=omega.kc, FUN=sum)
kcMonthlyNight <- aggregate(TOTAL.HOURS.WORKED ~ MONTH, data=omega.kc, FUN=sum)
kcMonthlyDay <- aggregate(TOTAL.DAY.HOURS ~ MONTH, data=omega.kc, FUN=sum)
kcMonthlyMaxEmpDay <- aggregate(INFERRED.DAY.EMPLOYEES ~ MONTH, data=omega.kc, FUN=max)
kcMonthlyMaxEmpDayActual <- aggregate(NUMBER.DAY.EMPLOYEES ~ MONTH, data=omega.kc, FUN=max)
kcMonthlyAvgDayEmp <- aggregate(INFERRED.DAY.EMPLOYEES ~ MONTH, data=omega.kc, FUN=mean)
kcMonthlyAvgDayEmpActual <- aggregate(NUMBER.DAY.EMPLOYEES ~ MONTH, data=omega.kc, FUN=mean)#
kcMonthlyMaxEmpNight <- aggregate(INFERRED.NIGHT.EMPLOYEES ~ MONTH, data=omega.kc, FUN=max)
kcMonthlyMaxEmpNightActual <- aggregate(NUMBER.OF.EMPLOYEES ~ MONTH, data=omega.kc, FUN=max)
kcMonthlyAvgNightEmp <- aggregate(INFERRED.NIGHT.EMPLOYEES ~ MONTH, data=omega.kc, FUN=mean)
kcMonthlyAvgNightEmpActual <- aggregate(NUMBER.OF.EMPLOYEES ~ MONTH, data=omega.kc, FUN=mean)#
kcMonthlyCasesRecd <- aggregate(INVENTORY.INFLOWS ~ MONTH, data=omega.kc, FUN=mean)
kcMonthlyAvgHoursPersonNight <- aggregate(AVG.HOURS.WORKED ~ MONTH, data=omega.kc, FUN=mean)
kcMonthlyAvgHoursPersonDay <- aggregate(AVG.DAY.HOURS ~ MONTH, data=omega.kc, FUN=mean)


kcMonthly <- merge(kcMonthlyCases, kcMonthlyNight, by='MONTH')#
kcMonthly <- merge(kcMonthly, kcMonthlyMaxEmpNight, by='MONTH')#
kcMonthly <- merge(kcMonthly, kcMonthlyMaxEmpNightActual, by='MONTH')#
kcMonthly <- merge(kcMonthly, kcMonthlyAvgNightEmp, by='MONTH')#
kcMonthly <- merge(kcMonthly, kcMonthlyAvgNightEmpActual, by='MONTH')#
kcMonthly <- merge(kcMonthly, kcMonthlyAvgHoursPersonNight, by='MONTH')#
kcMonthly <- merge(kcMonthly, kcMonthlyDay, by='MONTH')#
kcMonthly <- merge(kcMonthly, kcMonthlyMaxEmpDay, by='MONTH')#
kcMonthly <- merge(kcMonthly, kcMonthlyMaxEmpDayActual, by='MONTH')#
kcMonthly <- merge(kcMonthly, kcMonthlyAvgDayEmp, by='MONTH')#
kcMonthly <- merge(kcMonthly, kcMonthlyAvgDayEmpActual, by='MONTH')#
kcMonthly <- merge(kcMonthly, kcMonthlyAvgHoursPersonDay, by='MONTH')#
kcMonthly <- merge(kcMonthly, kcMonthlyCasesRecd, by='MONTH')

head(kcMonthly)
names(kcMonthly) <- c('MONTH', 'TOTAL.KC.CASES', 'TOTAL.NIGHT.HOURS', 'MAX.NIGHTLY.EMP.INFERRED',
...
differenceNight <- kcMonthly$MAX.NIGHTLY.EMP.ACTUAL - kcMonthly$MAX.NIGHTLY.EMP.INFERRED
kcMonthly$OVER.SHORT.NIGHT.STAFF <- ifelse(differenceNight > 0, paste('Over-staffed by', round(differenceNight, 1)), 
                                            paste('Under-staffed by', round(differenceNight, 1)))

differenceDay <- kcMonthly$MAX.DAILY.EMP.ACTUAL - kcMonthly$MAX.DAILY.EMP.INFERRED
kcMonthly$OVER.SHORT.DAY.STAFF <- ifelse(differenceDay > 0, paste('Over-staffed by', round(differenceDay, 1)), 
...
library(party)
model <- train(HRS.WORKED ~ TOTAL.STL.CASES + SALES.TAKES, data=omega.kc, 
...
# BEST FIT DAILY
bestFit <- lm(MAX.NIGHTLY.EMP.INFERRED ~ TOTAL.KC.CASES, data=kcMonthly)
summary(bestFit)
...
# MONTHLY
bestFit <- lm(MAX.DAILY.EMP.INFERRED ~ INVENTORY.INFLOWS, data=kcMonthly)
summary(bestFit)
names(kcMonthly)
