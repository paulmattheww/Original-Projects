...
stlWarehouseDayHours <- filter(hours, rule1 == "STL Warehouse Wages")

personHours <- aggregate(HOURS ~ DATE + PERSONNUM, data=stlWarehouseDayHours, FUN=sum)
personHours <- filter(personHours, PERSONNUM != 0755)

day.hours.stl <- aggregate(HOURS ~ DATE, data = personHours, FUN=sum)
day.hours.stl$DATE <- as.character(strptime(day.hours.stl$DATE, "%m/%d/%Y"))
names(day.hours.stl) <- c("DATE", "TOTAL.DAY.HOURS")

day.count.stl <- aggregate(HOURS ~ DATE, data = personHours, FUN=length)
day.count.stl$DATE <- as.character(strptime(day.count.stl$DATE, "%m/%d/%Y"))
names(day.count.stl) <- c("DATE", "NUMBER.DAY.EMPLOYEES")

check <- merge(day.hours.stl, day.count.stl, by="DATE")




# Read in sales takes, call code = CP, diver = invoice detail 3 year in folder, std cases & comp added
setwd("C:/Users/pmwash/Desktop/R_files")
salesTakes <- read.csv("sales-takes-2015.csv", header=TRUE)
names(salesTakes) <- c('DATE', 'WAREHOUSE', 'STD.CASES')
library(lubridate)
salesTakes$DATE <- as.character(strptime(salesTakes$DATE, format='%m/%d/%Y'))
salesTakes$YEAR <- year(salesTakes$DATE)
salesTakes <- filter(salesTakes, YEAR == 2015)
salesTakes <- filter(salesTakes, WAREHOUSE == 'ST LOUIS')
salesTakes$WAREHOUSE <- NULL
...
setwd("C:/Users/pmwash/Desktop/R_Files/Input Data")
...
OMEGA <- merge(OMEGA, day.count.stl, by="DATE")
OMEGA <- merge(OMEGA, salesTakes, by="DATE")
head(OMEGA)


# CREATE DUMMY VARIABLES FOR CORRELATIONS, THEN FILTER OUT 
library(dummies) #c('WEEKDAY','MONTH','SEASON')
...
# Eliminate day where we received 0 cases
OMEGA <- filter(OMEGA, CASES.RECEIVED > 5)

# Eliminate negative sales takes
OMEGA <- filter(OMEGA, SALES.TAKES > 0)

# Turn hours into People, 8 hours for day 10 hours for night
dayHoursAssumed <- 8
nightHoursAssumed <- 10
OMEGA$INFERRED.DAY.EMPLOYEES <- round(OMEGA$TOTAL.DAY.HOURS / dayHoursAssumed, 1)
OMEGA$INFERRED.NIGHT.EMPLOYEES <- round(OMEGA$HRS.WORKED / nightHoursAssumed, 1)

check <- cbind(OMEGA$INFERRED.NIGHT.EMPLOYEES, OMEGA$NUMBER.OF.EMPLOYEES)
check


# Avg daytime hours
OMEGA$AVG.DAYTIME.HOURS <- OMEGA$TOTAL.DAY.HOURS / OMEGA$NUMBER.DAY.EMPLOYEES
...
######################################################################################
######################################################################################
# Aggregate Monthly, Quarterly data ######################################################################################
######################################################################################

stlMonthlyCases <- aggregate(TOTAL.STL.CASES ~ MONTH, data=OMEGA, FUN=sum)
stlMonthlyNight <- aggregate(HRS.WORKED ~ MONTH, data=OMEGA, FUN=sum)
stlMonthlyDay <- aggregate(TOTAL.DAY.HOURS ~ MONTH, data=OMEGA, FUN=sum)
stlMonthlyMaxEmpDay <- aggregate(INFERRED.DAY.EMPLOYEES ~ MONTH, data=OMEGA, FUN=max)
stlMonthlyMaxEmpDayActual <- aggregate(NUMBER.DAY.EMPLOYEES ~ MONTH, data=OMEGA, FUN=max)
stlMonthlyAvgDayEmp <- aggregate(INFERRED.DAY.EMPLOYEES ~ MONTH, data=OMEGA, FUN=mean)
stlMonthlyAvgDayEmpActual <- aggregate(NUMBER.DAY.EMPLOYEES ~ MONTH, data=OMEGA, FUN=mean)#
stlMonthlyMaxEmpNight <- aggregate(INFERRED.NIGHT.EMPLOYEES ~ MONTH, data=OMEGA, FUN=max)
stlMonthlyMaxEmpNightActual <- aggregate(NUMBER.OF.EMPLOYEES ~ MONTH, data=OMEGA, FUN=max)
stlMonthlyAvgNightEmp <- aggregate(INFERRED.NIGHT.EMPLOYEES ~ MONTH, data=OMEGA, FUN=mean)
stlMonthlyAvgNightEmpActual <- aggregate(NUMBER.OF.EMPLOYEES ~ MONTH, data=OMEGA, FUN=mean)#
stlMonthlyCasesRecd <- aggregate(CASES.RECEIVED ~ MONTH, data=OMEGA, FUN=mean)
stlMonthlyAvgHoursPersonNight <- aggregate(HOURS.PER.DAY ~ MONTH, data=OMEGA, FUN=mean)
stlMonthlyAvgHoursPersonDay <- aggregate(AVG.DAYTIME.HOURS ~ MONTH, data=OMEGA, FUN=mean)


stlMonthly <- merge(stlMonthlyCases, stlMonthlyNight, by='MONTH')#
stlMonthly <- merge(stlMonthly, stlMonthlyMaxEmpNight, by='MONTH')#
stlMonthly <- merge(stlMonthly, stlMonthlyMaxEmpNightActual, by='MONTH')#
stlMonthly <- merge(stlMonthly, stlMonthlyAvgNightEmp, by='MONTH')#
stlMonthly <- merge(stlMonthly, stlMonthlyAvgNightEmpActual, by='MONTH')#
stlMonthly <- merge(stlMonthly, stlMonthlyAvgHoursPersonNight, by='MONTH')#
stlMonthly <- merge(stlMonthly, stlMonthlyDay, by='MONTH')#
stlMonthly <- merge(stlMonthly, stlMonthlyMaxEmpDay, by='MONTH')#
stlMonthly <- merge(stlMonthly, stlMonthlyMaxEmpDayActual, by='MONTH')#
stlMonthly <- merge(stlMonthly, stlMonthlyAvgDayEmp, by='MONTH')#
stlMonthly <- merge(stlMonthly, stlMonthlyAvgDayEmpActual, by='MONTH')#
stlMonthly <- merge(stlMonthly, stlMonthlyAvgHoursPersonDay, by='MONTH')#
stlMonthly <- merge(stlMonthly, stlMonthlyCasesRecd, by='MONTH')

names(stlMonthly) <- c('MONTH', 'TOTAL.STL.CASES', 'TOTAL.NIGHT.HOURS', 'MAX.NIGHTLY.EMP.INFERRED',
                       'MAX.NIGHTLY.EMP.ACTUAL', 'AVG.NIGHTLY.EMP.INFERRED', 'AVG.NIGHTLY.EMP.ACTUAL',
                       'AVG.HOURS.NIGHT.CREW.ACTUAL', 'TOTAL.DAY.HOURS', 'MAX.DAILY.EMP.INFERRED',
                       'MAX.DAILY.EMP.ACTUAL', 'AVG.DAILY.EMP.INFERRED', 'AVG.DAILY.EMP.ACTUAL',
                       'AVG.HOURS.DAY.CREW' ,'INVENTORY.INFLOWS')


differenceNight <- stlMonthly$MAX.NIGHTLY.EMP.ACTUAL - stlMonthly$MAX.NIGHTLY.EMP.INFERRED
stlMonthly$OVER.SHORT.NIGHT.STAFF <- ifelse(differenceNight > 0, paste('Over-staffed by', round(differenceNight, 1)), 
                                            paste('Under-staffed by', round(differenceNight, 1)))

differenceDay <- stlMonthly$MAX.DAILY.EMP.ACTUAL - stlMonthly$MAX.DAILY.EMP.INFERRED
stlMonthly$OVER.SHORT.DAY.STAFF <- ifelse(differenceDay > 0, paste('Over-staffed by', round(differenceDay, 1)), 
                                            paste('Under-staffed by', round(differenceDay, 1)))

write.csv(stlMonthly, 'stl_monthly_warehouse.csv')



# day shift 8 hour assumptions; night shift 10 hours
...
g + geom_point(size=5, aes(colour=SEASON)) + 
  facet_wrap(~SEASON, ncol=1) + 
  geom_smooth(method="lm", se=FALSE, size=1, colour='black',
              aes(group=SEASON)) +
  labs(title="Simple Regression, Hours Worked ~ Total Cases Produced + Season",
       x="TOTAL CASES PRODUCED STL", y="TOTAL HOURS WORKED (NIGHT)") +
...
#OMEGA <- OMEGA[,-2] 
names(OMEGA)
correl <- OMEGA[, c(2:3, 5:7, 13:28, 30:32, 34:42, 44:58)]
names(correl)
#correl <- correl[, c(2:6, 14:18, 20, 22:31)]
...
g + geom_histogram(binwidth=1.5, colour="orange", fill="lightgreen") +
  labs(title="Histogram of Keg Adjusted CPMH", 
       x="Keg Adjusted CPMH (bin width = 1.5)") +
  geom_vline(xintercept=c(32.54, 35.39, 37.52)) +
  annotate("text", x=30, y=23, 
label="25th percentile is 32.5") +
  annotate("text", x=35, y=25, label="50th percentile is 35.4") +
  annotate("text", x=40, y=23, label="75th percentile is 37.5")
...
# Check variables for near zero variance
library(caret)
nsv <- nearZeroVar(OMEGA, saveMetrics=TRUE)
nsv

# Rank variables in order of importance FOR NIGHT HOURS
library(randomForest)
control <- trainControl(allowParallel=TRUE, method='cv', number=4)
names(OMEGA)
quantVariables <- OMEGA[, -c(1, 4, 9:12, 33, 43)]
model <- train(HRS.WORKED ~., data=quantVariables, model='rf', trControl=control, importance=TRUE)
varImpPlot(model$finalModel, sort=TRUE, pch=6, col=1, cex=0.7)


library(party)
model <- train(HRS.WORKED ~ TOTAL.STL.CASES + SALES.TAKES, data=OMEGA, 
               model='rf', trControl=control, importance=TRUE)

control <- trainControl(allowParallel=TRUE, method='cv', number=4)

library(party)
model <- train(INFERRED.AVG.NIGHT.EMPLOYEES ~ TOTAL.STL.CASES, data=stlMonthly, 
               model='rf', trControl=control, importance=TRUE)
plot(ctree(model, data=stlMonthly), main='Conditional Inference Tree')
...
# nightly
bestFit <- lm(INFERRED.NIGHT.EMPLOYEES ~ TOTAL.STL.CASES, data=OMEGA)
summary(bestFit)


# monthly
bestFitMonthly <- lm(MAX.NIGHTLY.EMP.INFERRED ~ TOTAL.STL.CASES, data=stlMonthly)
summary(bestFitMonthly)



library(randomForest)
control <- trainControl(allowParallel=TRUE, method='cv', number=4)
quantVariables <- stlMonthly[, -1]
model <- train(MAX.NIGHTLY.EMP.INFERRED ~., data=quantVariables, model='rf', trControl=control, importance=TRUE)
varImpPlot(model$finalModel, sort=TRUE, xlim=10)
...
# daily
bestFitDay <- lm(INFERRED.DAY.EMPLOYEES ~ CASES.RECEIVED + SALES.TAKES, data=OMEGA)
summary(bestFitDay)
g <- ggplot(data=OMEGA, aes(x=SALES.TAKES, y=TOTAL.DAY.HOURS))
g + geom_point() + geom_smooth(method="lm")

# MONTHLY
bestFitMonthly <- lm(MAX.DAILY.EMP.INFERRED ~ INVENTORY.INFLOWS, data=stlMonthly)
summary(bestFitMonthly)
r2 <- summary(bestFitMonthly)$adj.r.squared

g <- ggplot(data=stlMonthly, aes(x=MONTH, y=INVENTORY.INFLOWS))
g + geom_bar(stat='identity')

g <- ggplot(data=stlMonthly, aes(x=INVENTORY.INFLOWS, y=MAX.DAILY.EMP.INFERRED))
g + geom_point(aes(colour=factor(MONTH)), size=5, aes(group=1)) + 
  geom_smooth(method='lm', colour='black', se=FALSE) +
  labs(title='Maximum Daily Warehouse Employees as a function of Monthly Inventory Inflows',
       x='Total Monthly Inventory Inflows', y='Maximum Number of Daily Employees in Given Month') +
  annotate("text", x=13500, y=15.75, label=paste('Adj. R-squared =', round(r2, 2))) +
  theme(legend.position='bottom')
...
library(randomForest)
control <- trainControl(allowParallel=TRUE, method='cv', number=4)
model <- train(TOTAL.DAY.HOURS ~., data=quantVariables, model='rf', trControl=control, importance=TRUE,
               preProc=c('center', 'scale'))
varImpPlot(model$finalModel, sort=TRUE, type=2, pch=6, col=1, cex=0.8)


library(party)
model <- train(TOTAL.DAY.HOURS ~ CASES.RECEIVED, data=OMEGA, 
               model='rf', trControl=control, importance=TRUE, 
               preProc=c('center', 'scale'))
library(evtree)
EV <- evtree(model, data=OMEGA)
plot(EV)
...
secondBestFit <- lm(TOTAL.DAY.HOURS ~ CASES.RECEIVED + CASES.INVENTORY.STL, data=OMEGA)
summary(secondBestFit)
g <- ggplot(data=OMEGA, aes(x=CASES.INVENTORY.STL, y=TOTAL.DAY.HOURS))
g + geom_point() + geom_smooth(method="lm")


checkFit <- lm(CASES.INVENTORY.STL ~ CASES.RECEIVED, data=OMEGA)
summary(checkFit)
...
fit <- lm(TOTAL.DAY.HOURS ~ CASES.RECEIVED + CASES.BROKEN, data=OMEGA)
summary(fit)


fit <- lm(TOTAL.DAY.HOURS ~ CASES.RECEIVED + SEASON, data=OMEGA)
summary(fit)


fit <- lm(TOTAL.DAY.HOURS ~ CASES.RECEIVED + END.OF.MONTH, data=OMEGA)
summary(fit)


fit <- lm(TOTAL.DAY.HOURS ~ CASES.RECEIVED + INVENTORY.STL.PRIOR.DAY, data=OMEGA)
summary(fit) #good fit


fit <- lm(TOTAL.DAY.HOURS ~ CASES.RECEIVED + CASES.BROKEN.YESTERDAY, data=OMEGA)
summary(fit)


fit <- lm(TOTAL.DAY.HOURS ~ CASES.RECEIVED.PLUS.CASES.SOLD, data=OMEGA)
summary(fit)


fit <- lm(O.T.HOURS ~ CASES.RECEIVED.PLUS.CASES.SOLD, data=OMEGA)
summary(fit)



OMEGA$SUM.CASES.DIVER.CHECK <- OMEGA$NA.STD.CASES + OMEGA$TOTAL.KEG.CASES + OMEGA$BEER.STD.CASES + 
  OMEGA$WINE.STD.CASES + OMEGA$LIQUOR.STD.CASES
head(OMEGA)
##################################################################################################
##################################################################################################
##################################################################################################
