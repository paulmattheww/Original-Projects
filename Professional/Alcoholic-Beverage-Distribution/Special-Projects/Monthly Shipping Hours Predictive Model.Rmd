...
##PREDICTIVE LABOR MODEL
p = ggplot(data=m, aes(x=Forecast.Production[,1], y=Night.Ship.Hours, group=House, label=Month))
CLOSER_LABOR_MODEL = p + geom_point(aes(colour=factor(House), size=Production.Days)) + 
  facet_wrap(House~Production.Days, ncol=3) + #, scales='free') +
  scale_y_continuous(labels=comma) +
  scale_x_continuous(labels=comma) +
  theme(legend.position='none') +
  geom_point(aes(x=Forecast.Production[,1], y=Forecast.Ship.Hours[,1])) +
  geom_line(aes(x=Forecast.Production[,1], y=Forecast.Ship.Hours[,1])) +
