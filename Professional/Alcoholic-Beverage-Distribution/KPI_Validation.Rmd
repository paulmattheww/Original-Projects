...
## Gather Data 
Each month's data was compiled from the daily reports filled out by Paul Cunningham. The actual code to compile this data is not included in this document due to its lack of relevance. 
...
After backing out all routes that are not attributed to drivers we see there was an increase of 1147 cases delivered YTD through August, which is an increase of 0.87%. These figures are consistent with Hogan's claims that stops have increased by ~1,000, but it does not appear to be a significant increase.
```{r}
library(dplyr)
...
Below is information on driver collections. From Hogan's own data, supplied by this years and last year's invoices under the metrics section, there was only a 0.86% increase in collections. 
```{r}
...
From internal data we see that the average cases per driver per day is 318.79 and the average number of stops per driver per day is 20.97. The average number of cases per stop is 40.6.
```{r}
