...
clean_up_groupby_cols = lambda cols: ['%s%s' % (a, '|%s' % b if b else '') for a, b in cols]
...
cal_df = pd.DataFrame()
for yr in yrz:
    cal_df.append(generate_calendar(year=yr))
...
call_code = {'01':'Customer Call','02':'ROE/EDI','03':'Salesperson Call','04':'Telesales','BH':'Bill & Hold',
                    'BR':'Breakage','CP':'Customer Pickup','FS':'Floor Stock','HJ':'Highjump','KR':'Keg Route',
                    'NH':'Non-Highjump','NR':'Non-Roadnet','PL':'Pallets','PR':'Personal','RB':'Redbull','SA':'Sample',
                    'SP':'Special','WD':'Withdrawal'}             
