...
MASTER_MANIFEST['BulkRoute'] = MASTER_MANIFEST.RouteIdentifier.apply(lambda x: str(x)[:1] == 'X')
...
MASTER_MANIFEST.head(2)


MASTER_MANIFEST[MASTER_MANIFEST.BulkRoute == True]





#MASTER_MANIFEST.groupby(['RouteIdentifier','RouteId'])[['OnTime_Weighted_RteDate','OnTime_RteDate']].mean()
...
## Put all of this into a portable thumb drive
## Enumerate all Qs for Pasha
## Figure out what info need to share
## Schedule call with Pasha
...
BYDAY_BYRTE = pd.DataFrame(MASTER_MANIFEST.groupby(['Date','RouteIdentifier','RouteId','RouteIdentifierWeekdayAgnostic','BulkRoute','CrossdockRoute']).agg(agg_funcs)).reset_index(drop=False)
BYDAY_BYRTE.head(25)
...
#agg_funcs = {'OnTime_Weighted_RteDate':np.max, 
             #'OnTime_RteDate':np.max, 
             #'Stops'}
#MASTER_MANIFEST.groupby(['RouteIdentifier','RouteId'])
